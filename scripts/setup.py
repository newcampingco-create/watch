...
Modes:
  ...
  setup.py              Setup helper. Prints dependency guidance and scaffolds .env.
...
- Never install system packages automatically. Print exact commands for the user.
...
def _which(name: str) -> str | None:
    if name == "yt-dlp" and (VENDOR_DIR / "yt_dlp" / "__main__.py").is_file():
        return str(VENDOR_DIR / "yt_dlp")
...
def _read_env_key(name: str) -> str | None:
    ...
    if value and value.strip():
        return value.strip()
    config_file = CONFIG_FILE if CONFIG_FILE.exists() else Path.cwd() / ".env"
...
def cmd_install() -> int:
    ...
    if missing:
        ...
        if system == "Darwin":
            print("[setup] dependencies missing on macOS — install only with user approval:", file=sys.stderr)
