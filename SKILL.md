---
name: watch
description: Watch and analyze a video from a URL or local path. Use when the user shares a video, asks what happens in a video, requests a summary or transcript-based analysis, or invokes /watch. Download with yt-dlp, extract timestamped frames with ffmpeg, obtain captions or a Whisper transcript, and answer from the visual and spoken content together.
---

# /watch

Use this skill to inspect video input. A Python script gets captions first, optionally downloads the video, extracts frames as JPEGs (scene-aware, or fast keyframes at `efficient` detail), gets a timestamped transcript (native captions first, then Whisper API as fallback), and prints frame paths. Inspect each frame as an image and combine it with the transcript to answer the user.
...
- **`first_run: true`** → genuine first-time setup. Do these in order:
  1. If `missing_binaries` is non-empty, run the setup helper to print platform-specific installation guidance. Do not install system packages without the user's explicit approval.
...
| Exit | Meaning | Action |
|------|---------|--------|
| `2` | Missing binaries (`ffmpeg` / `ffprobe` / `yt-dlp`) | Show the setup guidance and ask before installing anything |
...
| `4` | Both missing | Show setup guidance, then explain the optional transcription key |
...
The setup helper is idempotent — safe to re-run:

```bash
python3 "${SKILL_DIR}/scripts/setup.py"
```

It never installs system packages automatically. It prints the relevant commands and scaffolds `~/.config/watch/.env` with commented placeholders and default watch settings at `0600` permissions once dependencies are available.

**If an API key is still missing after setup:** explain that Groq or OpenAI transcription is optional. Never ask the user to paste an API key into chat and never write a key received through conversation. Ask them to set `GROQ_API_KEY` or `OPENAI_API_KEY` in their environment or edit `~/.config/watch/.env` themselves. If they decline, proceed with `--no-whisper` and explain that videos without native captions will be analyzed from frames only.
...
- **Setup preflight failed** → run `python3 "${SKILL_DIR}/scripts/setup.py"` to show dependency guidance and scaffold the `.env`. Ask before installing dependencies. For optional Whisper transcription, tell the user to configure a Groq or OpenAI key privately in their environment or local config; never request the key in chat.
...
**Bundled scripts:** `scripts/watch.py` (entry point), `scripts/download.py` (yt-dlp wrapper), `scripts/frames.py` (ffmpeg frame extraction), `scripts/transcribe.py` (caption selection + Whisper orchestration), `scripts/whisper.py` (Groq / OpenAI clients), `scripts/setup.py` (preflight + setup guidance)
